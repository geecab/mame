 ...
 ***************************************************************/
#define OP(prefix,opcode) void z80_device::prefix##_##opcode()
...
void z80_device::take_interrupt()
{
	...
	if( m_im == 2 )
	{
		...
		eat_cycles(CYCLES_ISR, m_cc_op[0xcd] + m_cc_ex[0xff]);
	}
	else
	...
	if( m_im == 1 )
	{
		...
		eat_cycles(CYCLES_ISR, m_cc_op[0xff] + cc_ex[0xff]);
	}
	else
	{
			...
			switch (irq_vector & 0xff0000)
			{
				case 0xcd0000:  /* call */
					...
					PCD = irq_vector & 0xffff;
						/* CALL $xxxx cycles */
					eat_cycles(CYCLES_ISR, m_cc_op[0xcd]);
					break;
				case 0xc30000:  /* jump */
					PCD = irq_vector & 0xffff;
					/* JP $xxxx cycles */
					eat_cycles(CYCLES_ISR, m_cc_op[0xc3]);
					break;
				default:        /* rst (or other opcodes?) */
					push(m_pc);
					PCD = irq_vector & 0x0038;
					/* RST $xx cycles */
					eat_cycles(CYCLES_ISR, m_cc_op[0xff]);
					break;
			}
		}

		/* 'interrupt latency' cycles */
		eat_cycles(CYCLES_ISR, m_cc_ex[0xff]);
	}
	WZ=PCD;
...
void z80_device::eat_cycles(int type, int cycles)
{
	// The 'type' argument is redundant here, but is used in the derived specz80 class.
	m_icount -= cycles;
}
